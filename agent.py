...
from config import FORM_DATA_FILE, AWS_REGION, BROWSER_ID
...
async def _run_with_retry(graph, build_msg_fn, phase_label, check_filled, on_success):
    messages = []
    failure = ""
    for attempt in range(1 + MAX_RETRIES):
        try:
            ...
            messages = await run_phase(graph, input_msgs, f"{phase_label}")
...
async def run_agent():
    try:
        ...
        logger.error("%s not found.", FORM_DATA_FILE)
        return
    ...
    async with get_bedrock_browser(AWS_REGION, BROWSER_ID) as page:
        ...
        for i, entry in enumerate(form_data, 1):
            ...
            # Phase 2 — Fill bill fields
            page_dump = ""
            async def after_bill():
                nonlocal page_dump
            ...
            # Phase 3 — Fill services rendered
